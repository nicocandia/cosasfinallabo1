...
void* ll_getNextIterator(LinkedList* this)
{
    Node* pNodo;
    if(this!=NULL){
        pNodo=this->nodeIterator;
        this->nodeIterator=pNodo->pNextNode;
        return this->nodeIterator;
    }
    return NULL;
}
...
void ll_map (LinkedList* this,void (*pFunc)(void*))
{
    ...
    if(this!= NULL && pFunc != NULL){
        pNodo=ll_startIterator(this);
        if(pNodo!=NULL){
            do{
                pFunc(pNodo->pElement);
                pNodo=ll_getNextIterator(this);
            }while(pNodo!=NULL);
        }
    }
}
